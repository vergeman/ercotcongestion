...
* `p_max_pu`: "maximum power output as a fraction of nameplate capacity." [0, 1]
  per-unit mulitiplier
* e.g for 100 MW wind farm: `p_nom` = 100; `p_max_pu` = 1 -> 100 MW right now,
  `p_max_pu` =.2 -> 20 MW right now
...
After the first two fixes, every bus had identical prices. Uniform LMPs mean no
binding transmission constraints. We verified that all line duals were genuinely
zero, TAMU network has enough line capacity that nothing was constraining the
optimum.

That meant no transmission congestion anywhere; unrealistic for Texas in August.
Just that TAMU dataset has enough line capacity that nothing was constraining
the optimum.

Fix: temporarily derate line capacities by 5% (×0.95) to force some binding
constraints. This stands in for N-1 reserve margins that real operators
maintain.

TODO: replace with real outage data from ERCOT.
