...
def apply_operating_conditions(n,
    ...
    if loads is not None:
        ...
        # default TAMU Texas2k case: no time series just static values
        load_vec = n.loads['p_set']
        n.loads_t.p_set = pd.DataFrame(
            [load_vec.values], index=n.snapshots, columns=n.loads.index
        )

    # Generator availability factors
    if p_max_pu_by_carrier:
