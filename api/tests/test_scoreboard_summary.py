...
DAILY = ScoreboardDaily(run_id="r", primary_source="model", points=[])
...
def test_summary_calls_each_section_with_its_existing_literal_defaults(monkeypatch):
    ...
    body = scoreboard_module.get_scoreboard_summary(regime="south")

    assert calls == {
        "weekly": ("model", "south", None),
        "headline": (None, "south"),
        "daily": (None, "model", None),
    }
...
def test_summary_turns_a_sections_503_into_a_null_field_without_failing_the_rest(
    ...
    body = scoreboard_module.get_scoreboard_summary(regime="all")
...
def test_summary_reraises_a_non_503_error(monkeypatch):
    ...
    try:
        scoreboard_module.get_scoreboard_summary(regime="all")
