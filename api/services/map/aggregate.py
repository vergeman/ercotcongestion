...
from schemas.map import MapMeta, MapOverview, OverviewConstraint, RankedConstraint, RankedConstraints, ReachSp
...
def overview(n: int, k: int, min_frac: float, *, coordinates: Coordinates, metadata: Metadata) -> MapOverview:
    with get_pool().connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        run_id, window_start = common.resolve(cur)
        ...
        if keys:
            ...
            for row in cur.fetchall():
                ...
                point_type, load_zone = metadata_rows.get(row["settlement_point"], (None, None))
                bucket.append(ReachSp(settlement_point=row["settlement_point"], sf=row["sf"], lat=lat, lon=lon, settlement_point_type=point_type, load_zone=load_zone))
        constraints = [OverviewConstraint(constraint_key=row["constraint_key"], ctype=row["ctype"], binding_hours=row["binding_hours"], max_abs_sf=row["max_abs_sf"], nodes=by_key[row["constraint_key"]]) for row in rows]
...
def ranked(day: date | None, basis: str, run_id: str | None, k: int, min_frac: float, *, coordinates: Coordinates) -> RankedConstraints:
    with get_pool().connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        if run_id is None:
        ...
        if day is None:
            ...
            if day is None:
                raise HTTPException(status_code=503, detail=f"no SF+μ artifact built for run_id={run_id}.")
        art = load_daily_artifact(cur, run_id, day)
        if art is None:
            raise HTTPException(status_code=503, detail=f"no SF+μ artifact for run_id={run_id} on {day}.")
        keys = art.SF.index
        if basis == "realized":
            lo, hi = art.E_mu.index.min(), art.E_mu.index.max()
            summary = pd.DataFrame.from_dict(_realized_mu_summary(cur, lo.to_pydatetime(), hi.to_pydatetime()), orient="index", columns=["mu_mass", "binding_hours"])
            mu_mass = summary.get("mu_mass", pd.Series(dtype=float)).reindex(keys).fillna(0.0)
            binding_hours = summary.get("binding_hours", pd.Series(dtype=float)).reindex(keys).fillna(0).astype(int)
        else:
    ...
    for rank, key in enumerate(top_keys, start=1):
        ...
        members = [(float(sf), coords.get(str(point))) for point, sf in sf_row.items() if abs(float(sf)) >= floor and sf != 0.0 and coords.get(str(point)) is not None]
        imports = sum(sf < 0 for sf, _ in members)
        exports = sum(sf > 0 for sf, _ in members)
        rows.append(RankedConstraint(constraint_id=key, rank=rank, congestion_contribution=float(ranking.loc[key]), mu_mass=float(mu_mass.loc[key]), binding_hours=int(binding_hours.loc[key]), reach=float(reach.loc[key]), n_members=len(members), ctype=geo.get(key, {}).get("ctype"), n_import=imports, n_export=exports))
    return RankedConstraints(run_id=run_id, delivery_date=day, basis=basis, k=k, n_ranked=int((contribution > 0.0).sum()), constraints=rows)
