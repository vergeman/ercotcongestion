...
def _windows(rows: list[dict], as_of: date) -> list[HeadlineWindow]:
    ...
    for days in WINDOWS:
        window_rows = [
            row for row in rows if as_of - timedelta(days=days) <= row["week"] <= as_of
        ]
        weeks = sorted({row["week"] for row in window_rows})
        currencies = []
        for name, higher_is_better in CURRENCIES:
            pooled = {
                source: _weighted_mean(
                    [
                        (row[name], row["n_hours"])
            ...
            currencies.append(
                HeadlineCurrency(
                    ...
                    climatology=pooled[_CLIMATOLOGY],
                    oracle=pooled[_ORACLE],
                    persistence_delta=(
                        None
                        if model is None or persistence is None
                        else model - persistence
                    ),
                )
            )
        result.append(
...
def build_headline(run_id: str | None) -> ScoreboardHeadline:
    with get_pool().connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        if run_id is None:
            ...
            run_id = row["run_id"]
        cur.execute(
            "SELECT week, source, n_hours, topdecile_hit, rank_spearman, sign_agree FROM scoreboard_weekly WHERE run_id = %s AND source = ANY(%s) ORDER BY week",
            (run_id, list(SOURCES)),
        )
        rows = cur.fetchall()
    if not rows:
        raise HTTPException(
            status_code=503,
            detail=f"no scoreboard_weekly rows for run_id={run_id}. Load the board first (compute.jobs.backfill_scoreboard).",
        )
    as_of = max(row["week"] for row in rows)
    return ScoreboardHeadline(
        run_id=run_id, as_of_week=as_of, windows=_windows(rows, as_of)
    )
