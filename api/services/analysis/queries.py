...
def constraints_response(
    *, artifact, geography: dict[str, dict], run_id: str, delivery_date, horizon: int
):
...
def node_response(
    *,
    ...
    if settlement_point not in artifact.SF.columns:
        raise HTTPException(
            status_code=404, detail="settlement point is absent from this artifact."
        )
    ...
    member_count = (
        essp_member_count(cur, settlement_point, selected[0].to_pydatetime())
        if include_detail and len(selected) == 1
        else None
    )
    return NodeAnalysisAvailableResponse(
        ...
        n_terms=(
            ...
            if mode == "structural"
            else int((contributions != 0.0).sum())
        ),
        ...
        structural_n_terms=(
            None if structural_sf is None else int((structural_sf != 0.0).sum())
        ),
        structural_terms=(
            None
