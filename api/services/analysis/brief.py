...
def _compose_brief_details(day: date, run_id: str, horizon: int | None, *, include_standouts: bool = True) -> BriefDetailsResponse:
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        sections = {
            "context": pool.submit(_timed_brief_section, "context", panels.get_context, day, run_id, horizon, 14),
            "top_nodes": pool.submit(_timed_brief_section, "top_nodes", panels.get_top_nodes, day, run_id, horizon, 10),
            "top_constraints": pool.submit(_timed_brief_section, "top_constraints", panels.get_top_constraints, day, run_id, horizon, 10),
            "grade": pool.submit(_timed_brief_section, "grade", panels.get_grade, day, run_id, horizon),
            "grade_history": pool.submit(_timed_brief_section, "grade_history", panels.get_grade_history, day, run_id, horizon, 30),
        }
        if include_standouts:
            sections["standouts"] = pool.submit(_timed_brief_section, "standouts", panels.get_standouts, day, run_id, horizon, 4)
...
def day(delivery_date: date | None, run_id: str | None, day: date | None = None) -> BriefDayResponse:
    ...
    with ThreadPoolExecutor(max_workers=7) as pool:
        sections = {
            "hero": pool.submit(_timed_brief_section, "hero", panels.get_hero, delivery_date, run_id, horizon),
            "context": pool.submit(_timed_brief_section, "context", panels.get_context, delivery_date, run_id, horizon, 14),
            "standouts": pool.submit(_timed_brief_section, "standouts", panels.get_standouts, delivery_date, run_id, horizon, 4),
            "top_nodes": pool.submit(_timed_brief_section, "top_nodes", panels.get_top_nodes, delivery_date, run_id, horizon, 10),
            "top_constraints": pool.submit(_timed_brief_section, "top_constraints", panels.get_top_constraints, delivery_date, run_id, horizon, 10),
            "grade": pool.submit(_timed_brief_section, "grade", panels.get_grade, delivery_date, run_id, horizon),
            "grade_history": pool.submit(_timed_brief_section, "grade_history", panels.get_grade_history, delivery_date, run_id, horizon, 30),
        }
        completed = {name: future.result() for name, future in sections.items()}
