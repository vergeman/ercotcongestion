...
def _brief_is_final(cur, delivery_date: date, horizon: int) -> bool:
    ...
    if horizon != 1 or delivery_date >= datetime.now(_CT).date():
        return False
    return analysis._dam_landed(cur, delivery_date)
...
def _compose_brief_details(day: date, run_id: str, horizon: int | None, *, include_standouts: bool = True) -> BriefDetailsResponse:
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        sections = {
            "context": pool.submit(_timed_brief_section, "context", analysis.get_context, day, run_id, horizon, 14),
            "top_nodes": pool.submit(_timed_brief_section, "top_nodes", analysis.get_top_nodes, day, run_id, horizon, 10),
            "top_constraints": pool.submit(_timed_brief_section, "top_constraints", analysis.get_top_constraints, day, run_id, horizon, 10),
            "grade": pool.submit(_timed_brief_section, "grade", analysis.get_grade, day, run_id, horizon),
            "grade_history": pool.submit(_timed_brief_section, "grade_history", analysis.get_grade_history, day, run_id, horizon, 30),
        }
        if include_standouts:
            sections["standouts"] = pool.submit(_timed_brief_section, "standouts", analysis.get_standouts, day, run_id, horizon, 4)
...
def hero_shell(delivery_date: date | None, run_id: str | None, day: date | None = None) -> BriefHeroShellResponse:
    ...
    with get_pool().connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        run_id = analysis._resolve_run(cur, run_id)
        horizon = analysis._resolve_horizon(cur, run_id, delivery_date, None)
    ...
    hero = analysis.get_hero(delivery_date, run_id, horizon, include_condition=False)
...
def hero_stats(delivery_date: date | None, run_id: str | None, day: date | None = None) -> BriefHeroStatsResponse:
    ...
    with get_pool().connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        if horizon == 1 and delivery_date < datetime.now(_CT).date():
            ...
            if cached is not None:
                return cached
    hero = analysis.get_hero(delivery_date, run_id, horizon)
...
def details(delivery_date: date | None, run_id: str | None, include_standouts: bool = True,
            day: date | None = None) -> BriefDetailsResponse:
...
def day(delivery_date: date | None, run_id: str | None, day: date | None = None) -> BriefDayResponse:
    ...
    with ThreadPoolExecutor(max_workers=7) as pool:
        sections = {
            "hero": pool.submit(_timed_brief_section, "hero", analysis.get_hero, delivery_date, run_id, horizon),
            "context": pool.submit(_timed_brief_section, "context", analysis.get_context, delivery_date, run_id, horizon, 14),
            "standouts": pool.submit(_timed_brief_section, "standouts", analysis.get_standouts, delivery_date, run_id, horizon, 4),
            "top_nodes": pool.submit(_timed_brief_section, "top_nodes", analysis.get_top_nodes, delivery_date, run_id, horizon, 10),
            "top_constraints": pool.submit(_timed_brief_section, "top_constraints", analysis.get_top_constraints, delivery_date, run_id, horizon, 10),
            "grade": pool.submit(_timed_brief_section, "grade", analysis.get_grade, delivery_date, run_id, horizon),
            "grade_history": pool.submit(_timed_brief_section, "grade_history", analysis.get_grade_history, delivery_date, run_id, horizon, 30),
        }
        completed = {name: future.result() for name, future in sections.items()}
