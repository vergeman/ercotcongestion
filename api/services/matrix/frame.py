...
class MatrixData(Protocol):
    ...
    def realized_mu(
        self, interval_ts: datetime, constraint_keys
    ) -> dict[str, float]: ...
    def constraint_types(self, keys: list[str]) -> dict[str, str]: ...


def _round(value: float) -> float:
    """Round a matrix display value to three decimal places."""
...
def _unavailable(
    ...
    """Build the empty response used when a frame cannot be served."""
    return MatrixFrame(
        available=False,
...
def build_frame(
    ...
    repository: MatrixData,
    metadata: dict[str, tuple[str | None, str | None]],
) -> MatrixFrame:
    """Load, select, and return the bounded Matrix frame for one cursor hour."""
    ...
    if context.artifact is None:
        return _unavailable(
            context.run_id, delivery_date, interval_ts, "artifact_missing"
        )
    artifact = context.artifact
    hour = pd.Timestamp(interval_ts)
    if hour not in artifact.E_mu.index:
        return _unavailable(
            context.run_id, delivery_date, interval_ts, "interval_not_in_artifact"
        )

    exact_mu = artifact.E_mu.loc[hour]
    dam_by_key = repository.realized_mu(interval_ts, artifact.SF.index)
    contribution = (
        artifact.E_mu.abs().sum(axis=0) * artifact.SF.abs().sum(axis=1)
    ).astype(float)
    contribution_ranked = [
        str(key)
        for key in sorted(
    ...
    if request.row_order == "anchor_contribution":
        ...
        ranked_rows = sorted(
            (str(key) for key in artifact.SF.index), key=lambda key: (-cursor[key], key)
        )
    else:
        ranked_rows = contribution_ranked

    pinned_rows = [
        key for key in request.pinned_constraints if key in artifact.SF.index
    ]
    pinned_columns = [
        key for key in request.pinned_settlement_points if key in artifact.SF.columns
    ]
    if (
        (peek := (request.peek_constraint or "").strip())
        and peek in artifact.SF.index
        and peek not in pinned_rows
    ):
        pinned_rows.append(peek)
    if (
        (peek := (request.peek_settlement_point or "").strip())
        and peek in artifact.SF.columns
        and peek not in pinned_columns
    ):
    ...
    if request.orientation == "nodes":
        row_keys, column_keys, col_max, row_types = select_nodes_major(
            ...
            pinned_rows,
            pinned_columns,
            request.settlement_point_search,
            request.row_limit,
            request.column_limit,
    ...
    for key in row_keys:
        ...
        rows.append(
            MatrixRow(
                ...
                forecast_mu=_round(float(exact_mu.loc[key])),
                ercot_dam_mu=dam_mu,
    ...
    columns = [
        MatrixColumn(
            ...
            max_abs_sf=_round(float(col_max.loc[key])),
        )
    ...
    contribution_day_max_abs = (
        float((artifact.E_mu.abs().max(axis=0) * artifact.SF.abs().max(axis=1)).max())
    ...
    return MatrixFrame(
        ...
        total_settlement_point_count=len(artifact.SF.columns),
