...
def append_bounded(base: list[str], additions: list[str], *, limit: int) -> list[str]:
    """Append unique items while preserving order and a fixed limit."""
    result = list(base)
...
def cursor_mu_abs(exact_mu, dam_by_key, keys) -> dict[str, float]:
    """Return each constraint's current absolute DAM or forecast value."""
    return {
        str(key): (
            abs(dam)
            if (dam := dam_by_key.get(str(key))) is not None
            else abs(float(exact_mu.loc[key]))
        )
        for key in keys
    }


def anchor_contribution_ranked(
    artifact, exact_mu, dam_by_key, anchors: list[str]
) -> list[str]:
    """Rank constraints by their current impact on the given anchor columns."""
...
def default_hub_column(artifact, metadata, row_keys: list[str]) -> str | None:
    """Choose the strongest hub column for the visible constraint rows."""
    hubs = [
        str(sp)
...
def select_constraints_major(
    ...
    """Return constraint rows, settlement-point columns, column scores, and types."""
    matched_rows = [
        key
        for key in ranked_rows
        if constraint_search and constraint_search in key.casefold()
    ][:MAX_SEARCH_RESULTS]
    type_candidates = append_bounded(
    ...
    return (
        row_keys,
        column_keys,
        artifact.SF.loc[default_rows].abs().max(axis=0),
        row_types,
    )
...
def select_nodes_major(
    ...
    """Return wire-format constraint rows and node columns for the node view."""
    cursor = cursor_mu_abs(exact_mu, dam_by_key, artifact.SF.index)
    default_constraint = (
        None
        if artifact.SF.empty
        else sorted(
            (str(k) for k in artifact.SF.index), key=lambda key: (-cursor[key], key)
        )[0]
    )
    base_rows = list(pinned_rows) or (
        [default_constraint] if default_constraint else []
    )
    row_keys = append_bounded([], base_rows, limit=max(column_limit, 1))
    row_keys = append_bounded(row_keys, pinned_rows, limit=MAX_ROW_LIMIT)
    row_types = constraint_types(row_keys)
    node_score = (
        artifact.SF.loc[row_keys].abs().max(axis=0)
        if row_keys
        else pd.Series(0.0, index=artifact.SF.columns)
    )
    ranked_nodes = [
        str(key)
        for key in sorted(
    ...
    column_keys = append_bounded(
        [], base_nodes, limit=max(MAX_COLUMN_LIMIT - len(additions), 1)
    )
    column_keys = append_bounded(column_keys, pinned_columns, limit=MAX_COLUMN_LIMIT)
