...
def _unavailable(
    run_id: str, delivery_date, interval_ts: datetime, reason: str
) -> MatrixFrame:
    return MatrixFrame(
        available=False,
...
def _bounded_values(values: list[str], *, name: str) -> list[str]:
    ...
    if len(normalized) > MAX_PINNED_ITEMS:
        raise HTTPException(
            status_code=422,
            detail=f"{name} supports at most {MAX_PINNED_ITEMS} values.",
        )
    return normalized
...
def _bounded_search(value: str | None, *, name: str) -> str | None:
    ...
    if len(value) > MAX_SEARCH_LENGTH:
        raise HTTPException(
            status_code=422,
            detail=f"{name} must be at most {MAX_SEARCH_LENGTH} characters.",
        )
...
def _constraint_types(cur, keys: list[str]) -> dict[str, str]:
    ...
    return {
        str(r["constraint_key"]): str(r["ctype"])
        for r in cur.fetchall()
        if r["ctype"] is not None
    }
...
def _anchor_contribution_ranked(
    artifact, exact_mu, dam_by_key, anchors: list[str]
) -> list[str]:
...
def _default_hub_column(artifact, metadata, row_keys: list[str]) -> str | None:
    ...
    Picks the hub settlement point with the largest max|SF| against the visible
    constraint rows, falling back to the first hub in the artifact when none
    score.

    """
    hubs = [
        str(sp)
        for sp in artifact.SF.columns
        if metadata.get(str(sp), (None, None))[0] == "hub"
    ]
    if not hubs:
        return None
    if row_keys:
...
def _select_constraints_major(
    ...
    constraint_search,
    constraint_type,
    settlement_point_search,
    row_limit,
    column_limit,
    row_preset,
    column_set,
):
    """Constraint-major selection: constraints are the primary (display-row)
    axis ranked by contribution; settlement points are the pinned/secondary
    columns.

    """
    matched_rows = [
        key
        for key in ranked_rows
        if constraint_search and constraint_search in key.casefold()
    ][:MAX_SEARCH_RESULTS]
    # Preserve the established DAM/type discovery order; both use bounded key sets.
    type_candidates = _append_bounded(
        ranked_rows[:MAX_ROW_LIMIT], pinned_rows, limit=MAX_ROW_LIMIT + MAX_PINNED_ITEMS
    )
    type_candidates = _append_bounded(
        type_candidates,
        matched_rows,
        limit=MAX_ROW_LIMIT + MAX_PINNED_ITEMS + MAX_SEARCH_RESULTS,
    )
    row_types = _constraint_types(cur, type_candidates)

    core_reference_limit = (
        row_limit
        if row_preset == "top30"
        else (DEFAULT_ROW_LIMIT if row_preset == "pinned" else ROW_PRESETS[row_preset])
    )
    ...
    ranked_columns = [
        str(key)
        for key in sorted(
            artifact.SF.columns, key=lambda key: (-core_max.loc[key], str(key))
        )
    ]
    ...
    base_rows = ranked_rows[
        : row_limit if row_preset == "top30" else ROW_PRESETS[row_preset]
    ]
    if constraint_type:
    ...
    anchor_columns = [
        key
        for key in ranked_columns
        if metadata.get(key, (None, None))[0] in {"hub", "load_zone"}
    ]
    ...
    matched_columns = [
        key
        for key in ranked_columns
        if settlement_point_search and settlement_point_search in key.casefold()
    ][:MAX_SEARCH_RESULTS]
    ...
    if (
        column_set == "pinned"
        and not pinned_columns
        and not settlement_point_search
        and not base_columns
    ):
        hub_seed = _default_hub_column(artifact, metadata, row_keys)
        if hub_seed is not None:
            base_columns = [hub_seed]
    additions = [
        key for key in pinned_columns + matched_columns if key not in base_columns
    ]
    column_keys = _append_bounded(
        [], base_columns, limit=MAX_COLUMN_LIMIT - len(additions)
    )
...
def _select_nodes_major(
    ...
    """Node-major selection: settlement points are the primary (display-row)
    axis ranked by |SF| reach against the shown constraints; constraints are
    the pinned/secondary columns seeded by the max-μ-at-cursor constraint.
    ...
    if not artifact.SF.empty:
        default_constraint = sorted(
            (str(k) for k in artifact.SF.index), key=lambda k: (-_cursor_mu(k), k)
        )[0]
    ...
    row_keys = _append_bounded(
        [], base_cols_c, limit=max(column_limit - len(additions_c), 1)
    )
    row_keys = _append_bounded(row_keys, pinned_rows, limit=MAX_ROW_LIMIT)
    ...
    ranked_nodes = [
        str(key)
        for key in sorted(
            artifact.SF.columns, key=lambda key: (-float(node_score.loc[key]), str(key))
        )
    ]
    matched_nodes = [
        key
        for key in ranked_nodes
        if settlement_point_search and settlement_point_search in key.casefold()
    ][:MAX_SEARCH_RESULTS]
    ...
    additions_n = [
        key for key in pinned_columns + matched_nodes if key not in base_nodes
    ]
    column_keys = _append_bounded(
        [], base_nodes, limit=max(MAX_COLUMN_LIMIT - len(additions_n), 1)
    )
    column_keys = _append_bounded(column_keys, pinned_columns, limit=MAX_COLUMN_LIMIT)
...
@router.get(
    "/frame", response_model=MatrixFrame, summary="Bounded causal SF matrix frame"
)
def get_matrix_frame(
    interval_ts: datetime = Query(
        ..., description="Delivery interval in ISO-8601 UTC."
    ),
    ...
    peek_constraint: str | None = Query(
        None,
        description='Force-include one previewed constraint as a row, beyond the pin cap (the working-set "top row" preview).',
    ),
    peek_settlement_point: str | None = Query(
        None,
        description="Force-include one previewed settlement point as a column, beyond the pin cap.",
    ),
    column_set: str = Query(
        "core",
        pattern="^(core|anchors|pinned|core_pinned|default_anchors)$",
        description="Bounded named column selection.",
    ),
    orientation: str = Query(
        "constraints",
        pattern="^(constraints|nodes)$",
        description="Which axis gets the primary ranked/searched list treatment.",
    ),
    row_order: str = Query(
        "contribution",
        pattern="^(contribution|cursor_mu|anchor_contribution)$",
        description="Constraint row selection order: day contribution; |DAM μ| (else |forecast μ|) at the cursor",
    ),
) -> MatrixFrame:
    pinned_constraint = _bounded_values(pinned_constraint, name="pinned_constraint")
    pinned_settlement_point = _bounded_values(
        pinned_settlement_point, name="pinned_settlement_point"
    )
    constraint_search = _bounded_search(constraint_search, name="constraint_search")
    settlement_point_search = _bounded_search(
        settlement_point_search, name="settlement_point_search"
    )
    interval_ts = _coerce_utc(interval_ts)
    # Artifacts are CT delivery-day blocks: the row keyed D spans 05:00Z D ->
    # 04:00Z D+1 in CDT (06:00Z -> 05:00Z in CST). The Central operating date
    # is therefore both the response label *and* the artifact key -- selecting
    # by UTC date sends the day's last five/six CT hours into the following
    # block, which begins after them.
    ...
    with get_pool().connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        if hour not in artifact.E_mu.index:
            return _unavailable(
                run_id, delivery_date, interval_ts, "interval_not_in_artifact"
            )
        ...
        sf_day_max_abs = (
            float(artifact.SF.abs().to_numpy().max()) if not artifact.SF.empty else 0.0
        )
        ...
        contribution_day_max_abs = (
            float(
                (artifact.E_mu.abs().max(axis=0) * artifact.SF.abs().max(axis=1)).max()
            )
            if not artifact.SF.empty
            else 0.0
        )
        ...
        pinned_columns = [
            key for key in pinned_settlement_point if key in artifact.SF.columns
        ]
        ...
        contribution_ranked = [
            str(key)
            for key in sorted(
                artifact.SF.index, key=lambda key: (-contribution.loc[key], str(key))
            )
        ]
        if row_order == "anchor_contribution":
            ...
            ranked_rows = _anchor_contribution_ranked(
                artifact, exact_mu, dam_by_key, anchors_present
            )
        elif row_order == "cursor_mu":
            cursor_abs = _cursor_mu_abs(exact_mu, dam_by_key, artifact.SF.index)
            ranked_rows = sorted(
                (str(key) for key in artifact.SF.index),
                key=lambda key: (-cursor_abs[key], key),
            )
        else:
    ...
    for key in row_keys:
        ...
        rows.append(
            MatrixRow(
                ...
                forecast_mu=_round_matrix_value(float(exact_mu.loc[key])),
                ercot_dam_mu=dam_mu,
    ...
    columns = [
        MatrixColumn(
            settlement_point=str(key),
            settlement_point_type=metadata.get(str(key), (None, None))[0],
    ...
    dam_status = (
        "available"
        if matched_dam == len(rows)
        else ("partial" if matched_dam else "pending")
    )
    values = [
    ...
    return MatrixFrame(
        ...
        total_constraint_count=len(contribution_ranked),
        total_settlement_point_count=len(all_columns),
        sf_day_max_abs=sf_day_max_abs,
        contribution_day_max_abs=contribution_day_max_abs,
        sf_abs_cap=SF_ABS_CAP,
        orientation=orientation,
        sf=MatrixSfValues(
            row_count=len(rows), column_count=len(columns), values=values
        ),
    )
