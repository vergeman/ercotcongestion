...
@router.get(
    "/brief/hero",
    response_model=BriefHeroShellResponse,
...
@router.get(
    "/brief/details",
    response_model=BriefDetailsResponse,
    summary="Secondary Brief sections for one delivery day",
)
def get_brief_details(
    delivery_date: date | None = Query(None, description="ERCOT delivery day."),
    run_id: str | None = Depends(_server_selected_run),
    include_standouts: bool = Query(
        True, description="Include the standouts panel in this bundle."
    ),
    *,
    day: date | None = Query(
        None, deprecated=True, description="Deprecated alias for delivery_date."
