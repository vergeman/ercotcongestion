...
* [x] `GET /analysis/brief/latest` returns the latest day's full brief envelope + `available_dates` (sorted); `GET /analysis/brief?delivery_date=` is unchanged.
* [x] `/analysis` renders the latest brief on load; `Analysis` nav item is enabled and active.
* [x] "Previous day" navigates to the prior date in `available_dates` and updates `?date=`; disabled at the earliest day. "Latest" returns to default.
* [x] Hour selector walks all 24 hours; defaults to the day's peak hour; a day-summary mode shows daily ranks + watchlist.
* [x] Best-pair story renders the separation, spread, dominance, and a driver waterfall that visibly sums to the spread (OLNEY↔LGW $159.19 on 2026-06-30 HE17). Endpoints read **source → sink** (low → high); the hour label is ERCOT hour-ending with the clock span, e.g. `HE17 · 4–5 PM CT`.
* [x] After-action renders the forecast→recon→actual decomposition + scorecard when present, and a "pending DAM" state when `after_action` is null.
* [x] F1–F4 render behind expanders from server values only; no client-side re-ranking. Horizon badge + `dam_match_coverage` shown in provenance.
* [x] Soft-fail: a day with no brief renders an empty state, never a thrown error.

### Post-plan additions (delivered)

* [x] **Routing single-sourced.** `lib/mapLinks.ts` owns the map's `?constraint=` / `?sp=` deep-link convention (parse + serialize + builders); `MapWorkspace` and `MatrixInspector` consume it instead of their own copies. `main.tsx` uses explicit named routes (`/map`, `/matrix`, `/scoreboard`, `/analysis`).
* [x] **HeaderNav routes client-side.** Nav links route through the Router (no full reload) for Map/Matrix/Analysis from any page, preserving the App-shell query on map↔matrix; **Scoreboard is the deliberate full-reload exception** (`reload` flag). Modifier/middle-click still opens a new tab.
* [x] **Best-pair nodes link to the map.** The source and sink settlement points are `<Link to={mapSettlementPointLink(sp)}>` → `/map?sp=…` (same-tab; new tab on modifier-click). Builders return a plain path so callers choose `<Link>` vs `target="_blank"`.

### Deferred (note, per last_mile.md)

* Multi-day list / pagination; strict-t+2 filter or t+1/t+2 toggle; LLM commentary. Revisit once the single-day slice proves the workflow.
* Broader cross-links: constraint/driver rows → `/map?constraint=…` (the helper `mapConstraintLink` exists; only the best-pair *nodes* are wired so far), and explicit "Show in Matrix" / "Show on Map" actions.
