...
* Add a regression test for the selector query shape; add `(run_id, delivery_date, ts DESC)` only if retained queries need it.
* Do NOT touch: score definitions, forecast publication order, or scoreboard API contracts.

## Acceptance

* [ ] Production selector completes promptly on the current `forecast_nodal` history and has no broad timestamp scan.
* [ ] It returns only an ungraded day whose final UTC hour has realized system lambda.
* [ ] Tests cover no candidate, partial realization, and newest gradeable-day selection.
* [ ] Logs distinguish selection time from grading time.
