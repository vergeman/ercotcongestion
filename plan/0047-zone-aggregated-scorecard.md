...
* [ ] `compute/mapping/scorecard.py` runs end-to-end on v1-120.
* [ ] `runs/v1-120/mapping/scorecard_v1-120.json` reports per-zone
  `{corr, sign_agreement, dispersion, n_buses, n_sps}` + window headline
  `{rank_spearman, mean_corr, mean_sign_agreement, n_hours}`.
* [ ] `runs/v1-120/mapping/scorecard_series_v1-120.npz` has aligned
  `hours`, `cluster_ids`, `model_Z`, `ercot_Z` arrays.
* [ ] `GET /validation?run_id=v1-120` returns the new schema; legacy
  Pearson calc, scatter, and `bus_load_zones` join removed from
  `api/validation.py`.
* [ ] `api/tests/` green.
* [ ] `web/src/api/types.ts` matches the new response schema; project
  compiles.
