...
* Preserve all forecast/map calculations, database schemas, artifact bytes, cron module paths, and existing public imports during migration.
...
* API, analysis, jobs, tests, and experiments directly import current `compute.sf.*` / `compute.mu.*` paths, so a big-bang rename would be operationally risky.
...
* Update `compute.jobs.*`, `compute.analysis.*`, `api/*`, and their tests to use only the new stage-level imports. Retain old `compute/sf/` and `compute/mu/` modules as deprecated, import-only compatibility facades for one release; do not move scheduled CLI module names in this plan.
...
* **4 — Canonicalize and document:** migrate experiments to stage APIs, mark legacy paths deprecated, update the code-tour documentation, and run the full focused suite. Do not remove compatibility façades in this plan.
...
* [x] Legacy import paths and deployed CLI paths continue to resolve through compatibility façades and produce unchanged results.
