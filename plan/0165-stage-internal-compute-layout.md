...
* [x] `mu_forecast` has panel, covariate, and model-execution packages; `mu_model.py` and `geo.py` no longer exist, and no compatibility import stubs remain.
...
* [x] Every production job, evaluation, experiment, probe, tutorial, and test uses canonical paths; no tracked source/docs refer to moved module paths.
* [x] Focused SF and μ test suites pass, with fixed-input SF, panel, μ prediction, and fixed-seed projection regression outputs unchanged.
