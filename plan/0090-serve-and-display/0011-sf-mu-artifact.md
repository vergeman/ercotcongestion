...
* **Commit F — `feat(propagate): --load-nodal-npz seeds a fresh DB from the npz-of-record`**
  * `propagate.py` — `--load-nodal-npz PATH` (requires `--run-id`) `COPY`s an existing panel npz into `forecast_nodal` and flips the pointer WITHOUT re-running the walk; the prod / fresh-DB catch-up path so the backtest computes once and the history moves to any DB as a bulk load (spec §5a).
...
* [x] `--load-nodal-npz PATH --run-id …` seeds `forecast_nodal` from an existing npz and flips the pointer without re-running the walk (fresh-DB / prod catch-up); requires `--run-id`, refuses the walk flags.
