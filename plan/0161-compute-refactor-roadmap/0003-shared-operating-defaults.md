...
* Make the adopted map/forecast window and refit defaults mechanically single-sourced.
* Retain semantic aliases where train and map terminology differ.

## Context

* `compute/sf/config.py` declares the adopted operating point, while `mu_model.py` repeats the matching `240` and `7` literals.
...
* [ ] No production 240-day or 7-day default is duplicated outside the canonical configuration and intentional experiment configs.
* [ ] Existing CLI help/default behavior remains unchanged.
* [ ] Focused μ, SF, and weather-default tests pass.

## Suggested regression tests

* Assert the μ aliases are object/value-equal to the SF configuration and that every affected argparse parser exposes the same numeric defaults as before.
