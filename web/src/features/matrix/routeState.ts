...
import type { MatrixEntitySelection, MatrixLens, MatrixTab, MatrixValTab } from "../../lib/matrix";
...
export function storedPins(): Pick<MatrixRouteState, "pinnedConstraints" | "pinnedSettlementPoints"> {
  try {
    ...
    if (value?.version === 1) return { pinnedConstraints: boundedPins(value.constraints ?? []), pinnedSettlementPoints: boundedPins(value.settlementPoints ?? []) };
  } catch { /* A malformed old value is recoverable through Reset. */ }
  return { pinnedConstraints: [], pinnedSettlementPoints: [] };
}

export function storedSeeded(): boolean {
  try { return (JSON.parse(window.localStorage.getItem(PIN_STORAGE_KEY) ?? "null") as { seeded?: boolean } | null)?.seeded === true; }
  catch { return false; }
}
...
export function matrixRouteStateFromSearch(search: string): MatrixRouteState {
  ...
  const tab: MatrixTab = tabParam === "nodes" || tabParam === "constraints" ? tabParam : sp && !constraintKey ? "nodes" : "constraints";
  ...
  return {
    tab,
    lens: lensParam === "sf" ? "sf" : lensParam === "basis" && tab === "nodes" ? "basis" : "read",
    val: valParam === "fmu" || valParam === "dmu" ? valParam : "sf",
    query: (params.get("q") ?? params.get("constraint_search") ?? "").slice(0, 64),
    fType: params.get("type") ?? "", fZone: params.get("zone") ?? "",
    ...
    basisA, basisB, basisActiveSlot: basisA && !basisB ? "b" : "a",
  };
}
...
export function useMatrixRouteState(routeSearch: string, onRouteChange: (search: string) => void) {
  const [state, setState] = useState<MatrixRouteState>(() => {
    const params = new URLSearchParams(routeSearch);
    const route = matrixRouteStateFromSearch(routeSearch);
    return {
      ...route,
      tab: params.has("tab") || params.has("sp") || params.has("constraint") ? route.tab : rememberedTab,
  ...
  const update = (patch: Partial<MatrixRouteState>) => setState((current) => {
    const next = { ...current, ...patch };
    rememberedTab = next.tab; rememberedLens = next.lens; rememberedVal = next.val;
    rememberedSelection = next.selection; rememberedBasisA = next.basisA; rememberedBasisB = next.basisB;
