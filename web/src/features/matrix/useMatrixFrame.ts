...
export function useMatrixFrame(timestamp: Date | null, state: MatrixRouteState, seeded: boolean, onSeed?: (frame: MatrixFrame) => void) {
  ...
  const previewConstraint = state.selection?.kind === "constraint" ? state.selection.key : null;
  const previewNode = state.selection?.kind === "node" ? state.selection.point : null;
  const peekConstraint = previewConstraint && !state.pinnedConstraints.includes(previewConstraint) ? previewConstraint : null;
  const peekSettlementPoint = previewNode && !state.pinnedSettlementPoints.includes(previewNode) ? previewNode : null;
  useEffect(() => {
    if (!timestamp) return;
    const controller = new AbortController();
    const id = ++requestId.current;
    const workingSetEmpty = !state.pinnedConstraints.length && !state.pinnedSettlementPoints.length;
    const seeding = !seeded && workingSetEmpty;
    setLoading(true); setError(null);
    const request = seeding
      ? { rowPreset: "top30" as const, rowLimit: 8, columnLimit: 30, columnSet: "default_anchors" as const, rowOrder: "anchor_contribution" as const }
      : { rowPreset: "pinned" as const, columnSet: "pinned" as const, rowLimit: 8, columnLimit: 30, pinnedConstraints: state.pinnedConstraints, pinnedSettlementPoints: state.pinnedSettlementPoints, peekConstraint, peekSettlementPoint };
