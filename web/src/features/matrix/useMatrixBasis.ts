...
export function useMatrixBasis(
  show: boolean,
  a: string | null,
  b: string | null,
  timestamp: Date | null,
  frame: MatrixFrame | null,
  basis: AnalysisBasis
) {
  ...
  useEffect(() => {
    if (!show || !a || !b || !timestamp || !frame?.delivery_date) {
      setPrimary(null);
      setRealizedTotal(null);
      setLoading(false);
      return;
    }
    ...
    setLoading(true);
    const fetchPair = (nextBasis: AnalysisBasis) =>
      Promise.all([
        getAnalysisNode(
          a,
          frame.delivery_date,
    ...
    void fetchPair(basis)
      .then(async ([first, second]) => {
        ...
        if (basis === "predicted" && frame.dam_status === "available") {
          try {
            nextRealized = basisFromNodes(
              ...(await fetchPair("realized"))
            ).total;
          } catch {
            /* Optional comparison. */
          }
