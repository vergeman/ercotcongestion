...
import {
  ...
  storedSeeded,
  useMatrixRouteState,
} from "../features/matrix/routeState";
...
export default function MatrixWorkspace({
  ...
  const { state, update } = useMatrixRouteState(
    routeSearch,
    onSelectionRouteChange
  );
  const [mobileIndexOpen, setMobileIndexOpen] = useState(false);
  const [seeded, setSeeded] = useState(storedSeeded);
  const { frame, loading, error, retry } = useMatrixFrame(
    timestamp,
    state,
    seeded,
    (nextFrame) => {
      setSeeded(true);
      update({
        pinnedConstraints: boundedPins(
          nextFrame.rows.map((row) => row.constraint_key)
        ),
        pinnedSettlementPoints: boundedPins(
          nextFrame.columns.map((column) => column.settlement_point)
        ),
      });
    }
  );
  ...
  const vocabulary = useMemo(
    () => matrixVocabulary(constraints, settlementPoints, state),
    [constraints, settlementPoints, state]
  );
  const showBasis = state.tab === "nodes" && state.lens === "basis";
  const basisType: AnalysisBasis =
    state.val === "dmu" ? "realized" : "predicted";
  const basis = useMatrixBasis(
    showBasis,
    state.basisA,
    state.basisB,
    timestamp,
    ...
    try {
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          settlementPoints: state.pinnedSettlementPoints,
        })
      );
    } catch {
      /* optional */
  ...
  const selectionKey =
    state.selection?.kind === "constraint"
      ...
      : null;
  const selectionInFrame = Boolean(
    selectionKey &&
      (state.tab === "nodes"
        ? frame?.columns.some(
            (column) => column.settlement_point === selectionKey
          )
        : frame?.rows.some((row) => row.constraint_key === selectionKey))
  );
  useEffect(() => {
    if (selectionInFrame && selectionKey) setFrameTopKey(selectionKey);
  }, [selectionInFrame, selectionKey]);
  ...
  const previewKey =
    topRowKey &&
    !isPinned(topRowKey, state.tab === "nodes" ? "sp" : "constraint")
      ? topRowKey
      : null;
  const selection =
    state.selection ??
    (vocabulary.fullItems[0]
      ? state.tab === "nodes"
        ? { kind: "node" as const, point: vocabulary.fullItems[0].id }
        : { kind: "constraint" as const, key: vocabulary.fullItems[0].id }
      : null);
  const selectedId =
  ...
  const selectedConstraint =
    selection?.kind === "constraint" && constraints?.available
      ? constraints.rows?.find((row) => row.constraint_key === selection.key) ??
        null
      : null;
  const selectedNode =
    selection?.kind === "node"
      ? vocabulary.nodeMeta.get(selection.point) ?? null
      : null;
  const usable = Boolean(
    frame?.available && frame.rows.length && frame.columns.length
  );
  const togglePin = (value: string, kind: "constraint" | "sp") => {
    const key =
      kind === "constraint" ? "pinnedConstraints" : "pinnedSettlementPoints";
    const values = state[key];
    update({
      [key]: isPinned(value, kind)
        ? values.filter((item) => item !== value)
        : boundedPins([value, ...values]),
    });
  };
  const select = (id: string) => {
    if (showBasis) {
      update(
        state.basisActiveSlot === "a"
          ? { basisA: id, basisActiveSlot: "b" }
          : { basisB: id, basisActiveSlot: "a" }
      );
      return;
    }
    update({
      selection:
        state.tab === "nodes"
          ? { kind: "node", point: id }
          : { kind: "constraint", key: id },
    });
  };
  const gridSelect = (next: MatrixSelection) => {
    if (next)
      update({
        selection:
          next.kind === "constraint"
            ? { kind: "constraint", key: next.constraintKey }
            : { kind: "node", point: next.settlementPoint },
      });
  };
  const reset = () => {
    setSeeded(false);
    ...
    <main className="matrix-workspace" aria-labelledby="matrix-title">
      ...
      {!error && usable && frame && (
        <div className="matrix-workspace__body">
          <MatrixSidebar
            tab={state.tab}
            items={vocabulary.filteredItems}
            totalCount={vocabulary.fullItems.length}
            selectedId={showBasis ? null : selectedId}
            basisSlots={
              showBasis ? { a: state.basisA, b: state.basisB } : undefined
            }
            onSelect={(id) => {
              select(id);
            ...
            onTab={(tab) =>
              update(
                tab === "constraints" && state.lens === "basis"
                  ? { tab, lens: "read" }
                  : { tab }
              )
            }
            ...
            fType={state.fType}
            fZone={state.fZone}
            typeOptions={vocabulary.typeOptions}
            zoneOptions={vocabulary.zoneOptions}
            onFilter={update}
            onTogglePin={(id) =>
              togglePin(id, state.tab === "constraints" ? "constraint" : "sp")
            }
            onReset={reset}
          ...
          <MatrixStage
            ...
            valueMode={matrixValueModeForVal(state.val)}
            muSource={matrixMuSourceForVal(state.val)}
