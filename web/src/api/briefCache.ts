...
export function fetchBriefHeroConditionCached(
  deliveryDate: string,
  runId?: string,
): Promise<BriefHeroCondition | null> {
  return cached(`brief-hero-condition:${deliveryDate}:${scope(runId)}`, () =>
    fetchBriefHeroCondition(deliveryDate, runId)
  );
}
