...
export default function GridMap({ topology, buses, meta, viewMode, onBusHover, onLineHover,
                                  onBusClick, onLineClick, onMapClick, selectedBusId, selectedLineId }: Props) {
    ...
    useEffect(() => {
        ...
        const onLoad = () => {
            if (!map.getSource('buses')) {
                map.addSource('buses', {
                    type: 'geojson', data: topo.buses as GeoJSON.FeatureCollection,
                    promoteId: 'bus_id'
                });
            }
            if (!map.getSource('lines')) {
                map.addSource('lines', {
                    type: 'geojson', data: topo.lines as GeoJSON.FeatureCollection,
                    promoteId: 'line_id'
                });
            }
                ...
                map.addLayer({
                    ...
                    paint: {
                        'line-color': [
                            ...
                            ['boolean', ['feature-state', 'binding'], false], '#f59e0b',
                            '#1e2d3e',
                        ],
                        'line-width': [
                            ...
                            4, ['case',
                                ...
                                ['boolean', ['feature-state', 'binding'], false], 1.5,
                                0.6],
                            8, ['case',
                                ['boolean', ['feature-state', 'selected'], false], 4.5,
                                ['boolean', ['feature-state', 'binding'], false], 3.0,
                                1.2],
                            12, ['case',
                                ['boolean', ['feature-state', 'selected'], false], 6.5,
                                ['boolean', ['feature-state', 'binding'], false], 5.0,
                                2.0],
                        ],
                        'line-opacity': [
                            'case',
    ...
    useEffect(() => {
        ...
        if (prevSelectedBusRef.current && prevSelectedBusRef.current !== selectedBusId) {
            map.setFeatureState(
        ...
        if (selectedBusId) {
            map.setFeatureState({ source: 'buses', id: selectedBusId }, { selected: true });
        }
    ...
    useEffect(() => {
        const map = mapRef.current;
        if (!map || !map.getSource('lines')) return;
        if (prevSelectedLineRef.current && prevSelectedLineRef.current !== selectedLineId) {
            map.setFeatureState(
