...
-- A day is "complete" only if ALL 6 endpoints have data for it.
-- =============================================================================
WITH days AS (
    SELECT generate_series(
        (SELECT LEAST(
            (SELECT MIN(interval_ts)::date    FROM ercot_zonal_lmp),
            (SELECT MIN(sced_timestamp)::date FROM shadow_prices),
...
SELECT
    ...
    EXISTS (SELECT 1 FROM shadow_prices          WHERE sced_timestamp::date = d.day) AS shadow,
    ...
    AND EXISTS (SELECT 1 FROM shadow_prices          WHERE sced_timestamp::date = d.day)
