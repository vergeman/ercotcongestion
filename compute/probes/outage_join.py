""" can outaged elements be joined to anonymous constraint keys?

Transmission-outage flags would be a high-value covariate for the mu-model, and
the join is the most uncertain part. Try to settles if we can use such data
before any outage work is scoped. The question has two legs:

  * **Leg A — does a constraint key carry a physical identity?** The keys
    (``constraint_name|contingency_name``) are opaque, but the NP4-191 rows
    behind them already carry ``from_station`` / ``to_station`` / ``kV``
    (``loaders.py:455``). Result: yes, it holds.

  * **Leg B — is there anything on the other side to join TO?** A join needs
    outage records that name transmission elements.

    Result: no such feed is reachable. Every public outage product is
    Resource-side: NP3-233 (hourly resource outage capacity, zonal, what we
    already ingest), NP1-346 (unplanned resource outages, 3-day lag),
    NP3-161/162 (planned-outage capacity margin).

    **Transmission** Outage Scheduler detail is not published; the reports that do
    exist sit behind the MIS Secure Area, which needs a Market Participant
    certificate this project doesn't hold.

The probe reports two rates; don't confuse them.

Leg A (``station_resolution_mass``) measures the constraint side only — can we
name a constraint's stations?

Leg B (``joinable_mass``) measures (scores) the actual join, and it's what the
bar checks.

Leg A can score high (0.976) while leg B is zero, because there's no outage
feed to join to.

Bar (fixed before the run), on leg B's ``joinable_mass``:

    >= 0.60  -> build the named outage covariate
    0.30-0.60 -> build it, flagged, for the joinable subset only
    <  0.30  -> fall back to ``outages_zonal`` (already ingested); no named covariate.

Leg C grades the fallback, outages_zonal.

``outages_zonal`` gives four load-zone MW columns — the same values for every
constraint in a zone. Each constraint could instead carry just its own zone's
number, if we knew which zone it sat in. The cheap way to find that (match
station names to settlement-point names) works for only ~9% of them, so the
real way would be the fitted SF map — but that needs a ``--persist-sf`` run and
isn't done here.
...
# Fixed before the run; not edited after.
R4_BUILD_BAR = 0.60
R4_FLAGGED_BAR = 0.30

# Outage tables in the DB, and the column that would name a transmission element.
# `outages_zonal` (NP3-233) is aggregate MW by load zone with no element column,
# which is why leg B fails. Kept as a lookup so that if a named feed is ever
# ingested, this probe finds it instead of hardcoding the verdict.
...
# All outage products found so far are Resource-side. A transmission product would
# have to match this pattern to change the verdict.
TRANSMISSION_HINT = re.compile(r"transmission|line|circuit|substation|element",
                               re.IGNORECASE)

# EMIL ids proposed as possible transmission-outage feeds, hit directly because
# being missing from the catalog and being missing from the API aren't the same
# claim. Both 404 on the live and archive endpoints, while NP4-158-SG (same '-SG'
# suffix, used as the control) returns 200 on the same credentials in the same
# pass — so this isn't an auth or rate-limit artifact, the products just aren't
# served publicly.
#
# Confirmed by the operator (2026-07-13): every ERCOT outage report lives behind
# the MIS Secure Area, which needs a Market Participant certificate, not the
# subscription key we use. So leg B is "the feed isn't reachable on our access
# path", not "we couldn't find it". Getting it would be a data-acquisition project
# (certificate + entitlement), not a modeling one.
...
def load_constraint_stations(conn, start: date, end: date) -> pd.DataFrame:
    ...
    ``start`` inclusive, ``end`` exclusive. Keyed the same way ``panels.py`` keys
    the fit (``constraint_name|contingency_name``) so the mass shares match.

    ERCOT writes empty strings, not NULLs, when a constraint has no station pair
    (the BASE CASE rows, e.g. ``E_PASP``). A ``NOT NULL`` test passes on every row
    and reports a false 100% resolution, so ``nullif(btrim(...), '')`` matters.

    ``n_pairs`` > 1 means the key's stations moved over the period (a re-rating or
    renamed element). Those keys still resolve; the pair just has to be carried
...
def station_resolution(df: pd.DataFrame) -> dict:
    ...
    Mass-weighted is the number that matters: reaching 95% of keys is worthless if
    they're the ones that never bind. Key-count share is reported beside it so that
    gap is visible.
    """
...
def named_outage_sources(conn) -> list[str]:
    ...
    Checked against the DB rather than assumed, so the day a named feed is ingested
    this probe reports a different verdict on its own.
    """
    found = []
...
def joinable_mass(df: pd.DataFrame, sources: list[str]) -> float:
    """The bar's currency: mu-mass joinable to a NAMED outage record.

    A join needs both legs. With no named transmission-outage source ingested, no
    amount of station resolution on the constraint side produces a joinable hour —
    hence zero, not 0.976. This function exists to make that gap impossible to fudge.
    """
    if not sources:
        return 0.0
    ...
    raise NotImplementedError(
        ...
        f"against them; the original verdict assumed none. Leg A caps the answer "
        f"at {resolved:.3f} of mu-mass."
    )
...
def check_catalog() -> pd.DataFrame:
    ...
    'ERCOT publishes no transmission-outage product' is a fact about a catalog, and
    catalogs change. This re-checks it in ~2 API calls so the verdict can be
...
def probe_candidates() -> pd.DataFrame:
    ...
    Being listed in the catalog and being served by the API are different claims,
    so the candidates are hit directly. The control is what makes a 404 mean
    something: without a known-served product answering 200 in the same pass on the
    same token, a wall of 404s could just mean a bad key.
    """
    import os
    import sys
    sys.path.insert(0, "/ercot_ingest")
...
def name_localization(conn, df: pd.DataFrame) -> dict:
    ...
    Not part of the R4 bar — this grades the fallback. ``outages_zonal`` gives four
    load-zone MW columns, the same four values for every constraint. If stations
    could be mapped to settlement points, each constraint could carry the outage MW
    of its own zone instead — a better covariate for the cost of just a string join.

    Routes tried: exact match, and station-as-prefix (``BIGLAKE`` ->
    ``BIGLAKE_UNIT1``). Both are weak — ERCOT's station names and settlement-point
    names are different naming schemes. The route that would work is electrical, not
    by name: the SPs with the largest ``|SF|`` for a constraint are its
    neighbourhood, straight off the fitted map. That needs a persisted SF run and is
    left for later.
    """
    with conn.cursor() as cur:
...
def verdict(joinable: float) -> tuple[str, str]:
    """The R4 decision. Bars are constants; this only reads them."""
    ...
    return "ZONAL_FALLBACK", (
        "fall back to the zonal aggregate already ingested (outages_zonal); "
        "proceed without a named outage covariate"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("\nThe join KEY is not the problem — leg A resolves "
          ...
          f"past the {R4_BUILD_BAR:.0%} bar.\nThe missing piece is the feed: ERCOT "
          "publishes no transmission-outage data, so there is nothing to join to.\n"
          "That is a data-availability finding, not a modelling one.")
