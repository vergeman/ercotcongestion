...
def build_panel(conn, M: pd.DataFrame, start, end,
    ...
    **The 0088 arms are opt-in, and both default to off**, so a caller that asks
    ...
    days = pd.DatetimeIndex(np.unique(availability.delivery_day_of(hours)))
    ...
    frame = pd.DataFrame({"interval_ts": hours,
                          "delivery_day": availability.delivery_day_of(hours)})
    panel = frame.merge(panel_engineering.downcast_join(hist.reset_index()), on="delivery_day",
                        how="inner")
    ...
    if C is not None and not C.empty:
        ...
        geo_panel(
            ...
            on_refit=lambda week_days, values: panel_engineering.attach_refit_features(
