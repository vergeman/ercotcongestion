...
Usage::
    ...
    python -m compute.legacy.run_pipeline \
        --run-id <name> \
...
def _stage_cmd(stage: str, args: argparse.Namespace) -> list[str]:
    ...
    if stage == "correlation_map":
        return base + [
            "compute.legacy.mapping.correlation_map",
            "--run-id", args.run_id,
        ]
    if stage == "basis_regression":
        return base + [
            "compute.legacy.mapping.basis_regression",
            "--run-id", args.run_id,
        ]
    if stage == "cca":
        return base + [
            "compute.legacy.mapping.cca",
            "--run-id", args.run_id,
        ]
    if stage == "clustering":
        cmd = base + [
            "compute.legacy.clustering.runner",
            "--run-id", args.run_id,
    ...
    if stage == "scorecard":
        return base + [
            "compute.legacy.mapping.scorecard",
            "--run-id", args.run_id,
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        prog="python -m compute.legacy.run_pipeline",
    ...
    ap.add_argument("--scorecard-ref", default="kkt_perbus",
                    help="Reference method the scorecard partitions against "
                         ...
                         "compute.legacy.mapping.correlation_map.DEFAULT_MODEL_REF).")
