...
`compute.legacy.clustering.render_partition` for a chosen `(algo, K)`.
...
`compute.legacy.mapping.diagnostics`).

CLI::

    python -m compute.legacy.clustering.runner \
        --matrices <npz> \
...
log = logging.getLogger("compute.legacy.clustering.runner")
...
def _load_beta_matrix(mapping_basis_path: Path | None) -> pd.DataFrame:
    ...
    if mapping_basis_path is None or not mapping_basis_path.exists():
        raise SystemExit(
            ...
            "Produce it with: python -m compute.legacy.mapping.basis_regression --run-id <run_id>"
        )
