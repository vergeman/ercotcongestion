...
def nodal_to_db(npz_path: str, conn, *, run_id: str,
    ...
    tracks live side by side (the preview is a preserved audit trail).
...
def sf_artifact_to_db(conn, *, run_id: str, delivery_date, sf_npz: bytes,
    ...
    `(run_id, delivery_date, horizon)`. Idempotent replace-in-place, the same
    discipline as `nodal_to_db` (0010): a re-run overwrites the day's blob for its
    horizon, never appends and never crosses horizons (0123). Does NOT commit — the
...
def persist_sf_mu_artifact(conn, SF: pd.DataFrame, E_mu: pd.DataFrame, *,
                           ...
                           horizon: int = 1) -> bytes:
    """Build the day's SF+μ blob once and land it per `(run_id, delivery_date, horizon)`.
