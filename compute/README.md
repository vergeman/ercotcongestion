...
Forecast publication writes `forecast_nodal` and `forecast_sf_artifact`, keyed
by `run_id`, delivery day, and horizon; `forecast_current` selects the served
run. The weekly scoreboard is stored in `scoreboard_weekly`. See
[`docs/ARTIFACTS.md`](../docs/ARTIFACTS.md) for the storage contract.
